...
for (cyr in syr2:eyr) {
  ...
  if (anomaly_assim){
    ...
    if (old_statvec) {
      if (precip_ratio) {
        echam_anom$data[echam_anom$names=='precip',,] <- echam$data[echam$names=='precip',,] / echam_clim$data[echam_clim$names=='precip',,]
        echam_anom$ensmean[echam_anom$names=='precip',] <- echam$ensmean[echam$names=='precip',] / echam_clim$ensmean[echam_clim$names=='precip',]
      } else {
    ...
    if (sixmonstatevector) {
      if ((real_proxies) & ((instrumental) | (docum))) { 
       ...
       #  Rcal[calibrate$sour=="doc"] <- 0.25 # equals 0.5 std. dev.
        ...
        # Rcal = (calibrate$data*0.1)^2 # use percentage to define the error when only precip is assimilated
        # Rcal <- c(precip=0.25)[calibrate$names] # used for GAprecipratio
        
        # # used it when assimilated temp, slp and precipitation ratio
        # Rcal=matrix(NA,nrow=length(calibrate$names),ncol=2)
        # Rcal[calibrate$names == "temp2"] = 0.9
        # Rcal[calibrate$names == "slp"] = 10
        # Rcal[calibrate$names =="precip"] = (calibrate$data[calibrate$names =="precip"]*0.1)^2
        
        Rcal <- calibrate$error^2
        
        # Rcal <- c(precip=50)[calibrate$names]
        #        if (avg_prox_per_grid) {Rcal <- Rcal*(1/calibrate$numavg)}
