...
calc_indices<-function(dataset, setname){
  ...
  if(setname=="echam2" | setname=="analysis2"){
    ...
    for (l in unique(dataset$lat)) {
      ...
      if (length(dataset$ensmean[pos,])==2) { zmean[c,] <- dataset$ensmean[pos,] }
      if (length(dataset$ensmean[pos,])>2) { zmean[c,] <- apply(dataset$ensmean[pos,],2,mean) }
      c=c+1
    }
    ...
    latmin1 <- gridmin1 - dlat * (((zmean[(gridminpos1-1),1])-(zmean[(gridminpos1+1),1]))/(2*(abs(zmean[gridminpos1]-max(c(zmean[(gridminpos1-1),1],zmean[(gridminpos1+1),1]))))))
    latmin2 <- gridmin2 - dlat * (((zmean[(gridminpos2-1),2])-(zmean[(gridminpos2+1),2]))/(2*(abs(zmean[gridminpos2]-max(c(zmean[(gridminpos2-1),2],zmean[(gridminpos2+1),2]))))))
    ...
    zmean <- zmean * -1 # because next equation is to find minimum and here we search for max
    sj <- sj * -1
    latmin1 <- gridmin1 - dlat * (((zmean[(gridminpos1-1),1])-(zmean[(gridminpos1+1),1]))/(2*(abs(sj[1]-max(c(zmean[(gridminpos1-1),1],zmean[(gridminpos1+1),1]))))))
    latmin2 <- gridmin2 - dlat * (((zmean[(gridminpos2-1),2])-(zmean[(gridminpos2+1),2]))/(2*(abs(sj[2]-max(c(zmean[(gridminpos2-1),2],zmean[(gridminpos2+1),2]))))))
    if (length(latmin1) > 1) {latmin1 <- mean(latmin1)}
    if (length(latmin2) > 1) {latmin2 <- mean(latmin2)}
    ...
    for (m in 1:nmem) {
      ...
      for (l in unique(dataset$lat)) {
        ...
        pos2 <- which(dataset$names=='omega500')
        pos <- intersect(pos1,pos2)
        if (length(dataset$data[pos,,m])==2) { zmean[c,] <- dataset$data[pos,,m] }
        if (length(dataset$data[pos,,m])>2) { zmean[c,] <- apply(dataset$data[pos,,m],2,mean) }
    ...
    for (m in 1:nmem) {
      ...
      if(setname=="echam2"){
        latmin1 <- gridmin1 - dlat * (((zmean[(gridminpos1-1),1])-(zmean[(gridminpos1+1),1]))/(2*(abs(itcz_tmp[1]-max(c(zmean[(gridminpos1-1),1],zmean[(gridminpos1+1),1]))))))
        latmin2 <- gridmin2 - dlat * (((zmean[(gridminpos2-1),2])-(zmean[(gridminpos2+1),2]))/(2*(abs(itcz_tmp[2]-max(c(zmean[(gridminpos2-1),2],zmean[(gridminpos2+1),2]))))))
      }else if(setname=="analysis2"){
        latmin1 <- gridmin1 - dlat * (((zmean[(gridminpos1-1),1])-(zmean[(gridminpos1+1),1]))/(2*(abs(itcz[1]-min(c(zmean[(gridminpos1-1),1],zmean[(gridminpos1+1),1]))))))
        latmin2 <- gridmin2 - dlat * (((zmean[(gridminpos2-1),2])-(zmean[(gridminpos2+1),2]))/(2*(abs(itcz[2]-min(c(zmean[(gridminpos2-1),2],zmean[(gridminpos2+1),2]))))))
      }
    ...
    for (m in 1:nmem) {
      ...
      zmean <- zmean * -1 # because next equation is to find minimum and here we search for max
      sj_tmp <- sj_tmp * -1
      latmin1 <- gridmin1 - dlat * (((zmean[(gridminpos1-1),1])-(zmean[(gridminpos1+1),1]))/(2*(abs(sj_tmp[1]-max(c(zmean[(gridminpos1-1),1],zmean[(gridminpos1+1),1]))))))
      latmin2 <- gridmin2 - dlat * (((zmean[(gridminpos2-1),2])-(zmean[(gridminpos2+1),2]))/(2*(abs(sj_tmp[2]-max(c(zmean[(gridminpos2-1),2],zmean[(gridminpos2+1),2]))))))
      if (length(latmin1) > 1) {latmin1 <- mean(latmin1)}
      if (length(latmin2) > 1) {latmin2 <- mean(latmin2)}
